...
class TestReporting:
    def test_build_leaderboard_aggregates_and_sorts_by_wer_then_rtf(self) -> None:
        rows = [
            _row(
    ...
    def test_report_csv_writers_persist_headers_and_rows(self, tmp_path: Path) -> None:
        rows = [
            _row(
                model_id="model",
                sample_id="sample",
        ...
        detailed_path = write_detailed_results_csv(rows, tmp_path / "run" / "detailed.csv")
        ...
        leaderboard_content = leaderboard_path.read_text(encoding="utf-8")
        assert detailed_content.splitlines()[0].startswith("run_id,model_id")
        assert "sample" in detailed_content
        assert "wer_mean" in leaderboard_content
        assert "model" in leaderboard_content
